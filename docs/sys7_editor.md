...
### MIGRATION mock alignment — design tokens + shared widgets (2026-05-02)

A high-fidelity React mock landed (`Downloads/UI_design/`) along with `MIGRATION.md`. The session's full plan and a post-mortem live at [`ui_migration_plan.md`](ui_migration_plan.md) and the 2026-05-02 entry of [`devlog.md`](devlog.md). The structural pieces that reach into `sys7` and survive the final state of the session:

- **Design tokens** (`engine/src/ui/StyleTokens.{h,cpp}`). 5-step black surface ladder (`BgVoid` `#000000` -> `BgBase` -> `BgPanel` -> `BgPanel2` -> `BgPanel3`), white-with-alpha borders/dividers, full neon accent palette (`Cyan` `#22e6ff`, `Magenta` `#ff3df0`, `Lime`, `Amber`, `Red`, `Violet`) with `*Dim` companions, and 4-step text ladder (`TextHi`/`TextMid`/`TextLow`/`TextDim`). `ApplyMusicGameStyle(ImGuiStyle&)` wires every `ImGuiCol_*` to a token. `WindowBg` set to `BgVoid` so editor pages render on a true-black canvas.
- **Shared widget library** (`engine/src/ui/Widgets.{h,cpp}`).
  - `ui::SectionHeader(label, rightSlot=lambda)` — uppercase-with-chevron section title; `rightSlot` is the per-section right-aligned action area (e.g. a `DefaultPill`). State persisted via `ImGuiStorage` keyed on label.
  - `ui::DefaultPill(id)` — small cyan-bordered ghost pill that resets a section's state to defaults. Right-slot of every Start Screen section.
  - `ui::Pill(label, color, solid=false)` — rounded pill for difficulty / mode badges. Solid = filled, outline = `WithAlpha(color, 0.16)` fill + `0.40` border.
  - `ui::LabeledChip(label, keyHint, accent, active)` — note-toolbar chip with colored swatch + label + boxed keyboard-hint digit. Used by `SongEditor::renderNoteToolbar`.
  - `ui::SegBar(...)` — segmented control, two overloads: string-id-keyed and int-indexed.
  - `ui::Dropdown(id, items, count, current)` — chevroned `BeginCombo` styled to match `SegBar`.
  - `ui::Slider(label, *v, min, max, suffix, accent)` — custom 6 px track + accent fill from min->value + white thumb with accent halo + mono right-aligned value readout. Replaces `ImGui::SliderFloat` on all editor pages.
  - `ui::PushMono()` / `ui::PopMono()` — push/pop the bundled `Cousine-Regular.ttf` (`ui::s_monoFont`, loaded by `ImGuiLayer::init`) for timecodes / paths / numbers.
- **Cross-page TopBar** (`ui::TopBar(crumbs, rightSlot)`). 56 px chrome with a gradient cyan->magenta `M` tile + breadcrumb labels joined by `>` + a right-slot lambda. Helpers: `ui::TopNavBack(label)`, `ui::TopNavForward(label)`, `ui::TopTestGame()` (lime primary + draw-list play triangle, CP936-safe — no glyph fonts). Wired on ProjectHub, StartScreen, MusicSelection, SongEditor. Each editor's `ImGui::Begin` flagged `NoTitleBar` + `WindowPadding=(0,0)` so the TopBar is the only chrome row.
- **ProjectHub mode metadata** (`ProjectInfo` in `engine/src/ui/ProjectHub.h`). New fields `gameMode` (`GameModeType`), `gameDim` (`DropDimension`), `songCount` (int) populated during `scanProjects()` by parsing the project's `music_selection.json` first song's `gameMode` block. Drives the new mode-pill column on the project table and the filter-pills row (`m_modeFilter`).
- **SongEditor 4-way Game Mode SegBar**. `renderGameModeConfig(structureOnly=true)` now drives a single `ui::SegBar({"Drop 2D", "Drop 3D", "Circle", "Scan Line"}, &modeIdx)`. The previous 3-button type picker + 2-button dimension picker is removed. Mode change still calls `exportAllCharts()` then `reloadChartsForCurrentMode()` so the chart-file split per mode is preserved.
- **NoteToolbar Audit right-align**. `renderNoteToolbar` ends with a `SameLine(0.f, remaining - 60.f)` call to push the `Audit` button to the right edge, matching MIGRATION §3.4's `Marker | <chips> | Analyze · Clr Mrk · Thin · Undo · Place · AI · Clr Note | Audit` order. `Clr Note` and `Audit` got token-aware outline-ghost styling (red and violet respectively) replacing the previous inline-pushed colors.
- **Settings — Cancel / Apply footer**. `SettingsPageUI::render` footer split: ghost `Cancel` calls `host.onBack()`, primary cyan-glow `Apply` calls `host.onSave()` then `host.onBack()`. Per-section accent labels (Audio cyan / Gameplay magenta / Visual lime / Misc amber) kept via the local `sectionHeader` helper (the bare `ui::SectionHeader` doesn't take an accent parameter; refactor deferred).

Visual fidelity against the mock is incomplete — Start Screen in particular has open layout deltas (TopBar chrome height + bg color, Hierarchy nav iconography, Background drop-zone striped placeholder, aspect-bar segmented control). Each was attempted during the session and reverted at the user's request; see the 2026-05-02 entry of `devlog.md` for the full sequence.
