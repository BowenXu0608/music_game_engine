    ...
    <Surface>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: 'var(--text-hi)', fontSize: 10, fontWeight: 700 }}>M</span>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0, flexDirection: sidebarLeft ? 'row' : 'row-reverse' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: 'var(--bg-void)' }}>
          ...
          <div style={{ flex: '1.4', minHeight: 0, position: 'relative', borderBottom: '1px solid var(--border)', background: '#000', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 5, display: 'flex', gap: 6, alignItems: 'center', padding: 4, background: 'rgba(13,13,16,0.85)', backdropFilter: 'blur(10px)', borderRadius: 6, border: '1px solid var(--border-hi)' }}>
              <AspectControl value="16:9" />
        ...
        {/* RIGHT — Copilot / Audit tabs (per §4.4) — collapsible */}
        {!copilotOpen && (
          <div
            ...
            style={{
              width: 30,
              borderLeft: sidebarLeft ? '1px solid var(--border)' : 'none',
              borderRight: sidebarLeft ? 'none' : '1px solid var(--border)',
              background: 'var(--bg-panel)',
              display: 'flex', flexDirection: 'column', alignItems: 'center',
              padding: '8px 0', gap: 10, cursor: 'pointer', userSelect: 'none', flex: '0 0 auto',
            }}>
            <div style={{ width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-mid)' }}>‹</div>
            <div style={{ width: 18, height: 18, borderRadius: 4, background: `linear-gradient(135deg, var(--${cfg.accent}), var(--magenta))`, boxShadow: `0 0 8px color-mix(in oklch, var(--${cfg.accent}) 40%, transparent)` }}></div>
            <div style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)', fontSize: 10, fontWeight: 600, letterSpacing: '0.2em', color: `var(--${cfg.accent})`, textTransform: 'uppercase', marginTop: 4 }}>Copilot · Audit</div>
          </div>
        )}
        {copilotOpen && (
        <div style={{ width: 280, borderLeft: sidebarLeft ? '1px solid var(--border)' : 'none', borderRight: sidebarLeft ? 'none' : '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
          <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
            ...
            <div onClick={() => setCopilotOpen(false)} title="Collapse Copilot" style={{ width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-mid)', cursor: 'pointer', fontSize: 14 }}>›</div>
          </div>
      ...
      {/* Pinned bottom Assets strip — shared across editors */}
      <AssetStrip />
    </Surface>
  );
}
...
function Drop2DScene() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0,
      background: 'radial-gradient(ellipse at 50% 100%, #0a0c0f 0%, #050507 60%, #000 100%)' }}>
      <svg viewBox="0 0 800 500" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        <defs>
          <linearGradient id="d2track" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="rgba(95,184,201,0)" />
            <stop offset="100%" stopColor="rgba(95,184,201,0.05)" />
          </linearGradient>
        </defs>
        {/* Drop 2D = vertical lanes, NOT perspective trapezoid */}
        <rect x="200" y="40" width="400" height="440" fill="url(#d2track)" stroke="rgba(95,184,201,0.14)" />
        {Array.from({length: lanes + 1}).map((_, i) => {
          const x = 200 + (i / lanes) * 400;
          const isEdge = i === 0 || i === lanes;
          return <line key={i} x1={x} y1={40} x2={x} y2={480}
            stroke={isEdge ? 'rgba(95,184,201,0.35)' : 'rgba(95,184,201,0.10)'}
...
function Drop3DScene() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0,
      background: 'radial-gradient(ellipse at 50% 100%, #100a0e 0%, #050507 60%, #000 100%)',
      perspective: 1000 }}>
      <div style={{ position: 'absolute', top: '15%', left: '50%', transform: 'translateX(-50%)', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(200,122,184,0.04)5), transparent 60%)', filter: 'blur(40px)' }}></div>
      <svg viewBox="0 0 800 500" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        {/* Perspective trapezoid */}
        <polygon points="320,80 480,80 700,500 100,500" fill="rgba(200,122,184,0.03)" stroke="rgba(200,122,184,0.14)" strokeWidth="1" />
        {Array.from({length: lanes + 1}).map((_, i) => {
          const f = i / lanes;
          const xTop = 320 + f * 160, xBot = 100 + f * 600;
          const isEdge = i === 0 || i === lanes;
          return <line key={i} x1={xTop} y1={80} x2={xBot} y2={500}
            stroke={isEdge ? 'rgba(200,122,184,0.35)' : 'rgba(200,122,184,0.10)'}
            strokeWidth={isEdge ? 1.5 : 1} />
        })}
        {/* Sky bar */}
        <line x1="280" y1="170" x2="520" y2="170" stroke="rgba(138,127,176,0.35)" strokeWidth="2" strokeDasharray="6 3" />
        <text x="540" y="174" fill="rgba(138,127,176,0.55)" fontSize="10" fontFamily="JetBrains Mono">sky h=1.0</text>
        {/* Judgment line */}
...
function CircleScene() {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0,
      background: 'radial-gradient(ellipse at 50% 50%, #0a100c 0%, #050507 70%, #000 100%)' }}>
      <svg viewBox="0 0 800 500" preserveAspectRatio="xMidYMid meet" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        ...
        {[80, 130, 180].map((r, i) => (
          <circle key={i} cx="400" cy="250" r={r} fill="none"
            stroke={i === 1 ? 'var(--lime)' : 'rgba(143,184,122,0.14)'}
            strokeWidth={i === 1 ? 2 : 1} />
        ))}
        {/* Inner radius shaded */}
        <circle cx="400" cy="250" r="80" fill="rgba(143,184,122,0.02)" stroke="rgba(143,184,122,0.25)" strokeDasharray="4 3" />
        {/* Hit ring (base radius) — glowing */}
        <circle cx="400" cy="250" r="130" fill="none" stroke="var(--lime)" strokeWidth="6" opacity="0.18" filter="blur(4px)" />
        {/* Lane spokes */}
        {Array.from({length: tracks}).map((_, i) => {
          ...
          return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(143,184,122,0.08)" strokeWidth="1" />;
        })}
        {/* Notes — radial */}
        {[
        ...
        {/* Center label */}
        <text x="400" y="254" textAnchor="middle" fill="rgba(143,184,122,0.5)" fontSize="11" fontFamily="JetBrains Mono">12 tracks</text>
      </svg>
    </div>
  );
}
...
function ScanLineScene() {
  return (
    <div style={{ position: 'absolute', inset: 0,
      background: 'radial-gradient(ellipse at 50% 50%, #100d0a 0%, #050507 70%, #000 100%)' }}>
      <svg viewBox="0 0 800 500" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
        {/* Page area — full canvas, lane-less */}
        <rect x="40" y="40" width="720" height="420" fill="rgba(212,165,116,0.025)" stroke="rgba(212,165,116,0.20)" strokeDasharray="4 3" />
        <text x="48" y="32" fill="rgba(212,165,116,0.55)" fontSize="10" fontFamily="JetBrains Mono">Page 7 · 1.62s · ↓ down sweep</text>
    ...
    <div style={{ height: 56, background: 'var(--bg-panel)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '0 10px', gap: 6, overflowX: 'auto' }}>
      ...
      {pages.map((p, i) => (
        <div key={i} style={{
          ...
          background: p.active ? 'rgba(212,165,116,0.10)' : 'var(--bg-panel-2)',
          border: `1px solid ${p.active ? 'var(--amber)' : 'var(--border)'}`,
          boxShadow: p.active ? '0 0 6px rgba(212,165,116,0.25)' : 'none',
...
function Waveform({ accent = 'cyan' }) {
  ...
  return (
    <div style={{ height: '100%', position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg, #0a0a0d, #050507)' }}>
      ...
      {markers.map((m, i) => (
        <div key={i} style={{ position: 'absolute', left: `${m*100}%`, top: 4, bottom: 4, width: 1, background: 'rgba(212,165,116,0.4)', borderTop: '4px solid var(--amber)', borderBottom: '4px solid var(--amber)' }}></div>
      ))}
