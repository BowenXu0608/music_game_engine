    ...
    <Surface>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <div style={{ width: 280, borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column' }}>
          <Tabs items={[{ id: 'p', label: 'Properties' }]} value="p" onChange={()=>{}} />
            ...
            <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 0, background: 'var(--bg-panel)', borderRadius: 4, padding: 2, border: '1px solid var(--border)' }}>
                  {['Drop 2D','Drop 3D','Circle','Scan Line'].map((m,i) => (
                    <div key={m} style={{
        ...
        {/* Copilot — page edit prompts */}
        <CopilotPanel
          accent="magenta"
          greeting="Describe a music-selection tweak — e.g. 'sort songs by difficulty', 'use frosted glass for the cover overlay'."
          placeholder="Ask Copilot to edit this page…"
        />
      </div>

      {/* Pinned bottom Assets strip */}
      <AssetStrip />
    </Surface>
  );
}
