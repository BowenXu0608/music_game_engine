...
// ───────────── Aspect ratio dropdown (shared by all scene windows) ─────────────
// Mirrors PreviewAspect.h preset list (landscape-only).
const ASPECT_PRESETS = [
  { label: '16:9',     w: 1920, h: 1080, note: 'Desktop / TV' },
  { label: '16:10',    w: 1920, h: 1200, note: 'Tablet landscape' },
  { label: '4:3',      w: 1440, h: 1080, note: 'iPad (pre-2024)' },
  { label: '3:2',      w: 1620, h: 1080, note: 'Surface / iPad Pro' },
  { label: '18:9',     w: 2160, h: 1080, note: 'Tall phone' },
  { label: '19.5:9',   w: 2340, h: 1080, note: 'iPhone X-class' },
  { label: '20:9',     w: 2400, h: 1080, note: 'Samsung / OnePlus' },
  { label: '21:9',     w: 2520, h: 1080, note: 'Ultrawide' },
  { label: '1:1',      w: 1080, h: 1080, note: 'Square' },
  { label: 'Custom',   w: 0,    h: 0,    note: 'Engine-stored w×h' },
];

const AspectControl = ({ value = '16:9', onChange }) => {
  const [open, setOpen] = React.useState(false);
  const cur = ASPECT_PRESETS.find(p => p.label === value) || ASPECT_PRESETS[0];
  const dims = cur.label === 'Custom' ? '— × —' : `${cur.w}×${cur.h}`;
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8 }}>
      <div
        ...
        style={{
          ...
          color: 'var(--text-hi)', cursor: 'pointer', userSelect: 'none',
          minWidth: 80,
        }}>
        <span style={{ width: 14, height: 10, border: '1px solid var(--cyan)', borderRadius: 1, opacity: 0.85, flex: '0 0 auto' }}></span>
        <span>{cur.label}</span>
        <span style={{ color: 'var(--text-low)', fontSize: 9, marginLeft: 'auto' }}>▾</span>
      </div>
      <span className="mono" style={{ fontSize: 10, color: 'var(--text-low)' }}>{dims}</span>
      {open && (
        <>
          <div onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 50 }}></div>
          <div style={{
            position: 'absolute', top: 26, left: 0, zIndex: 51,
            minWidth: 220, background: 'var(--bg-panel-2)',
            border: '1px solid var(--border-hi)', borderRadius: 5,
            boxShadow: '0 8px 24px rgba(0,0,0,0.6)',
            padding: 4, display: 'flex', flexDirection: 'column', gap: 1,
          }}>
            <div style={{ padding: '4px 8px', fontSize: 9, color: 'var(--text-low)', textTransform: 'uppercase', letterSpacing: '0.1em', borderBottom: '1px solid var(--divider)', marginBottom: 2 }}>
            ...
            {ASPECT_PRESETS.map(p => {
              ...
              return (
                <div key={p.label}
                  onClick={() => { onChange && onChange(p.label); setOpen(false); }}
                  style={{
                    padding: '5px 8px', borderRadius: 3, cursor: 'pointer',
                    background: active ? 'color-mix(in oklch, var(--cyan) 14%, transparent)' : 'transparent',
                    borderLeft: active ? '2px solid var(--cyan)' : '2px solid transparent',
                    display: 'flex', alignItems: 'center', gap: 8,
                  }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: active ? 'var(--cyan)' : 'var(--text-hi)', minWidth: 56 }}>{p.label}</span>
                  <span className="mono" style={{ fontSize: 10, color: 'var(--text-mid)', minWidth: 70 }}>
                    {p.label === 'Custom' ? '— × —' : `${p.w}×${p.h}`}
                  </span>
                  <span style={{ fontSize: 10, color: 'var(--text-low)', flex: 1, textAlign: 'right' }}>{p.note}</span>
                </div>
              );
            })}
          </div>
...
Object.assign(window, { ASPECT_PRESETS, AspectControl });


const AssetStrip = ({ defaultOpen = true }) => {
  const [open, setOpen] = React.useState(defaultOpen);
  const items = [
    { name: 'bg.png',         kind: 'tex', color: 'cyan' },
    { name: 'logo.png',       kind: 'tex', color: 'magenta' },
    { name: 'glow.png',       kind: 'tex', color: 'cyan' },
    { name: 'cover_neon.jpg', kind: 'tex', color: 'amber' },
    { name: 'loop_main.ogg',  kind: 'aud', color: 'lime' },
    { name: 'tap_start.wav',  kind: 'aud', color: 'lime' },
    { name: 'preview.ogg',    kind: 'aud', color: 'lime' },
    { name: 'forest.osu',     kind: 'chart', color: 'violet' },
    { name: 'lunar.arc',      kind: 'chart', color: 'violet' },
    { name: 'note_pulse.frag',kind: 'shd', color: 'cyan' },
    { name: 'bg_scroll.frag', kind: 'shd', color: 'cyan' },
    { name: 'ring.frag',      kind: 'shd', color: 'cyan' },
  ];
  const KIND_TINT = { tex: 'cyan', aud: 'lime', chart: 'violet', shd: 'magenta' };
  return (
    <div style={{
      ...
      <div onClick={() => setOpen(o => !o)} style={{
        height: 28, padding: '0 10px', display: 'flex', alignItems: 'center', gap: 8,
        borderBottom: open ? '1px solid var(--divider)' : 'none', cursor: 'pointer',
        userSelect: 'none',
      }}>
        <span style={{ fontSize: 9, color: 'var(--text-low)', width: 10 }}>{open ? '▾' : '▸'}</span>
        <span style={{ fontSize: 10, color: 'var(--text-mid)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>Assets</span>
        <span className="mono" style={{ fontSize: 10, color: 'var(--text-low)' }}>· {items.length} files · /Projects/BandoriSandbox/assets</span>
        <div style={{ flex: 1 }}></div>
        <div style={{ display: 'flex', gap: 6, fontSize: 10 }}>
          {Object.entries(KIND_TINT).map(([k, c]) => (
            <span key={k} style={{ color: `var(--${c})`, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{k}</span>
          ))}
        </div>
        <div style={{ width: 1, height: 14, background: 'var(--border)' }}></div>
        <Btn kind="ghost" size="sm" icon={I.plus}>Import</Btn>
      </div>
      {open && (
        <div style={{
          padding: '8px 10px',
          ...
          {items.map(it => (
            <div key={it.name} style={{
              flex: '0 0 88px', height: 92, display: 'flex', flexDirection: 'column', gap: 4,
              cursor: 'grab', userSelect: 'none',
            }}>
              <div style={{
                flex: 1, borderRadius: 4,
                background: `repeating-linear-gradient(135deg,
                  color-mix(in oklch, var(--${it.color}) 10%, #0a0a0d) 0 5px,
                  color-mix(in oklch, var(--${it.color}) 18%, #0a0a0d) 5px 10px)`,
                border: `1px solid color-mix(in oklch, var(--${it.color}) 30%, transparent)`,
                position: 'relative',
              }}>
                <span className="mono" style={{
                  position: 'absolute', top: 3, left: 4, fontSize: 8,
                  color: `var(--${it.color})`, textTransform: 'uppercase', letterSpacing: '0.08em',
                }}>{it.kind}</span>
              </div>
              <div className="mono" style={{ fontSize: 9, color: 'var(--text-mid)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.name}</div>
            </div>
          ))}
        </div>
      )}
...
// ───────────── Copilot side panel (right overlay; chart-edit prompts) ─────────────
const CopilotPanel = ({ accent = 'cyan', placeholder = 'Ask Copilot to edit this page…', greeting, withAudit = false }) => {
  ...
  if (!open) {
    return (
      <div
        ...
        style={{
          width: 30, borderLeft: '1px solid var(--border)',
          background: 'var(--bg-panel)', flex: '0 0 auto',
          display: 'flex', flexDirection: 'column', alignItems: 'center',
          padding: '8px 0', gap: 10, cursor: 'pointer', userSelect: 'none',
        }}
      >
        {/* Expand chevron */}
        <div style={{
          width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: 'var(--text-mid)', borderRadius: 4,
        }}>‹</div>
        {/* Copilot glyph */}
        <div style={{
          width: 18, height: 18, borderRadius: 4,
          background: `linear-gradient(135deg, var(--${accent}), var(--magenta))`,
          boxShadow: `0 0 8px color-mix(in oklch, var(--${accent}) 40%, transparent)`,
        }}></div>
        {/* Vertical label */}
        <div style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
        <div style={{ flex: 1 }}>
          <Tabs items={withAudit ? [{ id: 'c', label: 'Copilot' }, { id: 'a', label: 'Audit' }] : [{ id: 'c', label: 'Copilot' }]} value="c" onChange={() => {}} />
        </div>
        <div
          ...
          style={{
            ...
            color: 'var(--text-mid)', cursor: 'pointer', fontSize: 14,
          }}
        >›</div>
      </div>
    <div style={{ flex: 1, overflow: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0 }}>
      <div style={{ alignSelf: 'flex-start', maxWidth: '92%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
          <div style={{ width: 16, height: 16, borderRadius: 4, background: `linear-gradient(135deg, var(--${accent}), var(--magenta))` }}></div>
          <span style={{ fontSize: 9, color: `var(--${accent})`, fontWeight: 600, letterSpacing: '0.05em' }}>COPILOT</span>
        </div>
        <div style={{ padding: 8, borderRadius: 5, background: 'var(--bg-panel-2)', border: '1px solid var(--border)', fontSize: 11, color: 'var(--text-mid)', lineHeight: 1.4 }}>
          {greeting || 'Describe an edit in plain language. I\'ll preview the change, you Apply or Undo.'}
        </div>
      </div>
    </div>
    <div style={{ padding: 8, borderTop: '1px solid var(--divider)' }}>
      <div style={{ background: 'var(--bg-panel-2)', border: '1px solid var(--border-hi)', borderRadius: 5, padding: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{ flex: 1, fontSize: 11, color: 'var(--text-low)' }}>{placeholder}</span>
        <IconBtn icon={I.send} size={22} color={`var(--${accent})`} glow />
      </div>
    </div>
  </div>
  );
