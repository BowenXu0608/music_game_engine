...
## 2a. Shared editor chrome (cross-screen primitives)

Three elements are shared by every editor scene (Start Screen / Music Selection / Song Editor). Implement once as ImGui widgets and reuse — do not duplicate per-screen.

### Aspect ratio control (`AspectControl`)

A single dropdown pinned to the top-right of every scene preview toolbar. Backed by `PreviewAspect.h`'s preset list (landscape only, 10 entries):

| Label | W × H | Target |
|---|---|---|
| 16:9 | 1920×1080 | Desktop / TV |
| 16:10 | 1920×1200 | Tablet landscape |
| 4:3 | 1440×1080 | iPad (pre-2024) |
| 3:2 | 1620×1080 | Surface / iPad Pro |
| 18:9 | 2160×1080 | Tall phone |
| 19.5:9 | 2340×1080 | iPhone X-class |
| 20:9 | 2400×1080 | Samsung / OnePlus |
| 21:9 | 2520×1080 | Ultrawide |
| 1:1 | 1080×1080 | Square |
| Custom | engine-stored | manual w×h |

State is shared — `Engine::previewAspect()` is the single source of truth across all three scenes.

### Collapsible Copilot rail (`CopilotPanel`)

Right-side sidebar. Two states: **expanded** (280 px, chat + composer) and **collapsed** (30 px vertical rail with chevron + accent glyph + rotated label). Per-screen, session-only.

The **Audit** tab is **only on Song Editor**. Start Screen and Music Selection show Copilot alone.

### Pinned Asset strip (`AssetStrip`)

Full-width strip pinned to the bottom of every editor scene. Collapsible. Header: kind legend (TEX / AUD / CHART / SHD) + Import. Tray: horizontal cards tinted by kind. Does **not** also appear as a Properties tab.
