...
- Scan Line editor paginated redesign: one-sweep-per-page scene with Prev/Next + PgUp/PgDn nav, per-page `ScanPageOverride` speed multiplier (load-time expanded into synthetic `ScanSpeedEvent`s — `CytusRenderer` unchanged), direction alternates per page; integrates `AudioAnalyzer` beats (dashed orange ticks per page, click snap-to-nearest with `Alt` override, `Place All` scan-X alternating {0.25,0.5,0.75}); cross-page holds via mouse-wheel + Prev/Next in await-end, cross-page slides accumulate nodes with per-node `pageIndex`; cursor-follow scan line with `t=M:SS.sss` label; edge auto-page-turn (10px threshold, one-shot arm, disabled during playback); page auto-advances to match `curTime` on playback/scrub; Analyze Beats + Clear Markers + Select tools added to scan-line scene toolbar row (2026-04-17)
- Beat markers persist per chart file across all modes: `ChartData::markers` field round-trips through `exportAllCharts` + `ChartLoader::loadUnified`; `m_diffMarkers[diff]` hydrated from `chart.markers` on load; marker-only difficulties now also export (2026-04-17)
- APK packaging prune: `ProjectHub::startApkBuild` stages a copy of the project into `%TEMP%/<name>_apk_stage_<ts>/`, deletes every `assets/charts/<song>_*.json` whose basename isn't referenced by `music_selection.json` (keeps only the currently-selected mode per song), hands the staging path to `build_apk.bat`, and `renderApkDialog` cleans up the staging dir when the async build completes. Live editor project under `Projects/<name>/` is never modified (2026-04-17)
