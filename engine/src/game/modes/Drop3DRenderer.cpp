...
// Slot ids mirror MaterialSlots.cpp::kDrop3DSlots.
enum Drop3DSlot : uint16_t {
    SlotClickNote    = 0,
    SlotFlickNote    = 1,
    SlotArcTapTile   = 2,
    SlotArcTapShadow = 3,
...
Material Drop3DRenderer::slotOrFallback(uint16_t slot, const Material& fallback) const {
...
void Drop3DRenderer::onInit(Renderer& renderer, const ChartData& chart,
    ...
    // Derive the playfield-plane dimensions from the config BEFORE the meshes
    // build below (ground/gate/tap all reference these).
    m_laneHalfWidth = m_camCfg.playfieldWidth * 0.5f;
    m_laneFarZ      = -m_camCfg.playfieldLength;
    ...
    // configured trackCount (mirrors Drop2DRenderer — charts may be authored
    ...
    {
        const float xL    = -m_laneHalfWidth;
        const float xR    =  m_laneHalfWidth;
    ...
    for (const auto& note : chart.notes) {
        if (note.type == NoteType::ArcTap) {
            if (auto* tap = std::get_if<TapData>(&note.data)) {
                float wx = (tap->laneX * 2.f - 1.f) * m_laneHalfWidth;
...
void Drop3DRenderer::onResize(uint32_t w, uint32_t h) {
    m_width  = w;
    m_height = h;
    float aspect = static_cast<float>(w) / static_cast<float>(h);

    // Free 3D camera from the song config, shared with Drop2DRenderer and the
    // editor preview via buildGameplayCamera.
    m_camera = buildGameplayCamera(m_camCfg, aspect);
}

void Drop3DRenderer::onUpdate(float dt, double songTime) {
...
glm::vec2 Drop3DRenderer::evalArc(const ArcData& arc, float t) const {
    ...
    float wx = (nx * 2.f - 1.f) * m_laneHalfWidth;
    float wy = GROUND_Y + ny * m_skyHeight;
    return {wx, wy};
}

Mesh Drop3DRenderer::buildDynamicArcMesh(Renderer& renderer) {
...
void Drop3DRenderer::writeArcVertices(ArcMesh& am, float tClip) {
...
Mesh Drop3DRenderer::buildDynamicArcShadowMesh(Renderer& renderer) {
...
void Drop3DRenderer::writeArcShadowVertices(ArcMesh& am, float tClip) {
...
Mesh Drop3DRenderer::buildArcTapMesh(Renderer& renderer) {
    // Flat rectangular prism suspended in the sky — matches the Drop3D
...
Mesh Drop3DRenderer::buildArcTapShadowMesh(Renderer& renderer) {
...
Mesh Drop3DRenderer::buildTapMesh(Renderer& renderer) {
    // Size the tap to fit its lane slot — otherwise edge-lane taps bleed
    // past the lane's near-edge corner (half-width > half-slot).
    float slotHalf = m_laneHalfWidth / static_cast<float>(std::max(1, m_laneCount));
...
void Drop3DRenderer::onRender(Renderer& renderer) {
    ...
    for (auto& note : m_tapNotes) {
        ...
        if (z < 0.f || z > m_camCfg.playfieldLength) continue;
        float laneX = 0.f;
        if (auto* tap = std::get_if<TapData>(&note.data))       laneX = tap->laneX;
        else if (auto* fl = std::get_if<FlickData>(&note.data)) laneX = fl->laneX;
        else continue;

        float laneSpacing = (2.f * m_laneHalfWidth) / static_cast<float>(std::max(1, m_laneCount));
    ...
    for (auto& note : m_holdNotes) {
        ...
        if (zTail < 0.f || zHead > m_camCfg.playfieldLength) continue;
        float laneSpacing = (2.f * m_laneHalfWidth) / static_cast<float>(std::max(1, m_laneCount));
    ...
    // for ArcTaps (per Drop3D convention) — skip them.
    ...
    for (auto& am : m_arcs) {
        ...
        if (zOffset > m_camCfg.playfieldLength) continue;
...
void Drop3DRenderer::onShutdown(Renderer& renderer) {
...
void Drop3DRenderer::showJudgment(int /*lane*/, Judgment /*judgment*/, float /*timingDelta*/) {
    // Particle emission lives in showHitEffect (it carries NoteType, so each
    // Drop3D note type — tap / flick / hold / arc / arctap — gets its own
    // effect). Judgment TEXT for Drop3D is not wired yet.
}

void Drop3DRenderer::showHitEffect(HitEventKind kind, int lane,
                                   NoteType type, Judgment judgment) {
    if (!m_renderer || judgment == Judgment::Miss) return;

    // Particles transform through the active camera's viewProj; Drop3D's camera
...
void Drop3DRenderer::resolveParticleEffects(const GameModeConfig* config) {
    m_particleEffects.clear();
    if (!m_particleLibrary || !m_renderer) return;
    const std::string mode = "drop3d";
    for (const auto& slot : drop3dParticleSlots()) {
