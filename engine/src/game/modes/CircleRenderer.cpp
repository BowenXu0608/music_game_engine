...
namespace {
// Slot ids mirror MaterialSlots.cpp::kCircleSlots.
enum CircleSlot : uint16_t {
    SlotArcTile         = 0,
    SlotArcHalo         = 1,
    SlotHoldBody        = 5,
...
glm::vec4 CircleRenderer::slotTint(uint16_t slot, glm::vec4 fallbackRGBA) const {
...
Material CircleRenderer::slotMat(uint16_t slot, glm::vec4 fallbackRGBA) const {
    auto it = m_chartMaterials.find(slot);
...
glm::vec2 CircleRenderer::w2s(glm::vec3 pos, const glm::mat4& vp, float sw, float sh) {
...
void CircleRenderer::onInit(Renderer& renderer, const ChartData& chart,
    ...
    // CircleRingData notes — e.g. a chart authored in Drop-Notes mode that the
    ...
    std::cout << "[CircleRenderer] onInit trackCount=" << trackCount
    ...
    for (auto& note : chart.notes)
        if (auto* rd = std::get_if<CircleRingData>(&note.data))
    ...
    if (ringNotes.empty()) {
        ...
        for (auto& note : chart.notes) {
            ...
            converted.data = CircleRingData{angle, 0, span};
...
void CircleRenderer::reloadDiskAnimation(const DiskAnimation& anim) {
...
void CircleRenderer::onResize(uint32_t w, uint32_t h) {
    m_width  = w;
    m_height = h;
    rebuildPerspVP();
...
void CircleRenderer::rebuildPerspVP() {
...
// -----------------------------------------------------------------------------
float CircleRenderer::applyEasing(float t, EasingType easing) {
    switch (easing) {
...
glm::vec2 CircleRenderer::getDiskCenter(double songTime,
...
float CircleRenderer::getCurrentRotation(double songTime,
...
float CircleRenderer::getDiskScale(double songTime,
...
void CircleRenderer::onUpdate(float /*dt*/, double songTime) {
    m_songTime = songTime;
...
void CircleRenderer::buildRingPolyline(float radius, std::vector<glm::vec2>& out) const {
...
void CircleRenderer::onRender(Renderer& renderer) {
    ...
    for (auto& ring : m_rings) {
        ...
        for (auto& note : ring.notes) {
            ...
            auto* rd = std::get_if<CircleRingData>(&note.data);
            if (!rd) continue;
...
static float evalHoldLaneSmoothCircle(const HoldData& h, float tOff) {
...
void CircleRenderer::drawHoldBodies(Renderer& renderer) {
    ...
    for (const auto& hb : m_holdBodies) {
        ...
        // ── Circle-style hold body ─────────────────────────────────────────
        // In real Circle a hold body is a curved 2D track laid out on the
        ...
        for (int i = 0; i <= N; ++i) {
            ...
            float lane = evalHoldLaneSmoothCircle(h, tOff);
        ...
        // beam meets the disk edge. Real Circle holds keep this lens-shaped
        ...
        // tessellated curved sector that hugs the ring.
        {
            float headTOff = std::clamp(static_cast<float>(absStart - headT),
                                        0.f, h.duration);
            float headLane = evalHoldLaneSmoothCircle(h, headTOff);
...
void CircleRenderer::onShutdown(Renderer& renderer) {
    m_rings.clear();
    m_hitNotes.clear();
    m_holdBodies.clear();
    m_renderer = nullptr;
...
void CircleRenderer::markNoteHit(uint32_t noteId) {
    m_hitNotes.insert(noteId);
}

bool CircleRenderer::projectNoteScreen(uint32_t noteId, glm::vec2& outScreen) const {
    float sw = static_cast<float>(m_width);
    float sh = static_cast<float>(m_height);
    for (const auto& ring : m_rings) {
        for (const auto& note : ring.notes) {
            if (note.id != noteId) continue;
            const auto* rd = std::get_if<CircleRingData>(&note.data);
            if (!rd) return false;
            int   span        = std::clamp(rd->laneSpan, 1, 3);
            float laneAngular = (m_trackCount > 0 ? TWO_PI / m_trackCount : TWO_PI / 7.f);
            float angle    = rd->angle + ring.currentAngle
...
std::optional<CircleRenderer::PickResult>
CircleRenderer::pickNoteAt(glm::vec2 screenPx, double songTime, float pixelTol) const {
    float sw = static_cast<float>(m_width);
    float sh = static_cast<float>(m_height);
...
NoteType CircleRenderer::noteTypeById(uint32_t id) const {
    for (const auto& ring : m_rings)
        for (const auto& note : ring.notes)
...
void CircleRenderer::resolveParticleEffects(const GameModeConfig* config) {
    m_particleEffects.clear();
    if (!m_particleLibrary || !m_renderer) return;
    const std::string mode = "circle";
    for (const auto& slot : circleParticleSlots()) {
...
void CircleRenderer::emitHitFeedback(uint32_t noteId, Judgment judgment) {
...
CircleRenderer::findNoteByAngle(float targetAngle, float angularTol) const {
    const float HIT_WINDOW_SEC = 0.15f;
...
void CircleRenderer::showJudgment(int lane, Judgment judgment, float /*timingDelta*/) {
    ...
    // No standalone note at this lane/time — this is a Drop2D-style hold
...
uint32_t CircleRenderer::computeEnabledLanesAt(double songTime) const {
