...
class Drop2DRenderer : public GameModeRenderer {
public:
...
private:
    // Resolve the particle effect bound to each Drop2D event slot (from the
    ...
    float m_laneSpacing  = 1.2f;    // world units between lane centres (= playfieldWidth/laneCount)
    static constexpr float HIT_ZONE_Z   = 0.f;
    // Far draw/cull distance in world Z (= -playfieldLength). More negative =
    // the playfield plane extends further toward the vanishing point.
    float m_farCullZ = -110.f;
