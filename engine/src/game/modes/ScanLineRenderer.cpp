...
namespace {
// Slot ids mirror MaterialSlots.cpp::kScanLineSlots.
enum ScanLineSlot : uint16_t {
    SlotTapNote      = 0,
    SlotHoldBody     = 1,
    SlotHoldHead     = 2,
    SlotHoldTailCap  = 3,
...
glm::vec4 ScanLineRenderer::slotTint(uint16_t slot, glm::vec4 fallbackRGBA) const {
    ...
    // white). ScanLine draws this slot via the colour-only path, so surface the
    // base colour here.
    if (it->second.cls == MaterialClass::Pbr) return it->second.pbr.baseColor;
    return it->second.tint;
}

Material ScanLineRenderer::slotMat(uint16_t slot, glm::vec4 fallbackRGBA) const {
    auto it = m_chartMaterials.find(slot);
...
float ScanLineRenderer::scanLinePeriod() const {
    float bpm = m_bpm > 0.f ? m_bpm : 120.f;
    return 240.0f / bpm;
}

float ScanLineRenderer::applyDiskEasing(float t, DiskEasing e) {
...
void ScanLineRenderer::buildPhaseTable() {
    m_phaseTable.clear();
...
double ScanLineRenderer::interpolatePhase(double t) const {
...
float ScanLineRenderer::scanLineFrac(double t) const {
    double phase;
...
bool ScanLineRenderer::scanLineGoingUp(double t) const {
    double phase;
    if (m_phaseTable.empty()) {
        const double T = m_basePeriod;
...
void ScanLineRenderer::onInit(Renderer& renderer, const ChartData& chart,
...
void ScanLineRenderer::onResize(uint32_t w, uint32_t h) {
    m_width  = w;
    m_height = h;
    m_camera = Camera::makeOrtho(0.f, (float)w, (float)h, 0.f);
}

void ScanLineRenderer::onUpdate(float dt, double songTime) {
    m_songTime = songTime;
    for (auto& note : m_notes)
        if (note.isHit) note.hitTimer += dt;
}

void ScanLineRenderer::onRender(Renderer& renderer) {
    ...
    // Note quads. The PBR quad pipeline renders nothing under ScanLine's 2D
    ...
    // in pbr.baseColor, so this is visually identical to the pre-PBR ScanLine
    ...
    // ScanLine-style page visibility. Each sweep is a page; notes are shown
...
std::optional<ScanLineRenderer::PickResult>
ScanLineRenderer::pickNoteAt(glm::vec2 screenPx, double songTime, float pixelTol) const
{
...
void ScanLineRenderer::markNoteHit(uint32_t noteId) {
    for (auto& n : m_notes) {
        if (n.id == noteId) {
            n.isHit    = true;
            n.hitTimer = 0.f;
...
void ScanLineRenderer::showJudgment(int lane, Judgment judgment, float /*timingDelta*/) {
...
void ScanLineRenderer::resolveParticleEffects(const GameModeConfig* config) {
    m_particleEffects.clear();
    if (!m_particleLibrary || !m_renderer) return;
    const std::string mode = "scanline";
    for (const auto& slot : scanlineParticleSlots()) {
...
void ScanLineRenderer::emitSlideTickEffect(glm::vec2 screenPos, Judgment judgment) {
...
bool ScanLineRenderer::slideExpectedPos(uint32_t noteId, double songTime,
...
std::vector<ScanLineRenderer::SlideTick> ScanLineRenderer::consumeSlideTicks(double songTime) {
...
void ScanLineRenderer::onShutdown(Renderer& /*renderer*/) {
    m_notes.clear();
}
