...
// Slot ids mirror MaterialSlots.cpp::kDrop2DSlots.
enum Drop2DSlot : uint16_t {
    SlotTapNote        = 0,
    SlotHoldBody       = 1,
...
Material Drop2DRenderer::slotOrFallback(uint16_t slot, const Material& fallback) const {
...
glm::vec2 Drop2DRenderer::w2s(glm::vec3 pos, const glm::mat4& vp, float sw, float sh) {
...
float Drop2DRenderer::pxSize(float worldSz, float clipW, float proj11y, float sh) {
    if (clipW <= 0.f) return 0.f;
    return worldSz * proj11y * sh * 0.5f / clipW;
}

void Drop2DRenderer::onInit(Renderer& renderer, const ChartData& chart,
    ...
    // Copy the free-camera config; onResize builds the view/projection from it.
    if (config) {
        m_camCfg    = *config;
...
void Drop2DRenderer::resolveParticleEffects(const GameModeConfig* config) {
    m_particleEffects.clear();
    if (!m_particleLibrary || !m_renderer) return;

    const std::string mode = "drop2d";
    for (const auto& slot : drop2dParticleSlots()) {
...
glm::vec2 Drop2DRenderer::laneHitPos(float lane) const {
...
void Drop2DRenderer::onResize(uint32_t w, uint32_t h) {
    m_width  = w;
    m_height = h;

    float aspect = h > 0 ? static_cast<float>(w) / h : 1.f;

    // Free 3D camera from the song config (position + Euler rotation +
    // projection + clip planes). The playfield is a plane in world space; this
    // camera simply looks at it. Shared with Drop3DRenderer and the editor
    // preview via buildGameplayCamera so they can never diverge.
    Camera persp = buildGameplayCamera(m_camCfg, aspect);
    m_perspVP  = persp.viewProjection();
    m_proj11y  = std::abs(persp.projection()[1][1]);

    // Plane dimensions come straight from the config in world units. Playfield
    // Length alone bounds the drawn runway (the far clip plane is fixed and far
    // beyond it).
    m_laneSpacing = m_camCfg.playfieldWidth / std::max(1, m_laneCount);
    m_noteWorldW  = m_laneSpacing;
    m_farCullZ    = -m_camCfg.playfieldLength;

    m_camera = Camera::makeOrtho(0.f, static_cast<float>(w),
                                  static_cast<float>(h), 0.f);
}

void Drop2DRenderer::onUpdate(float dt, double songTime) {
    m_songTime = songTime;
...
void Drop2DRenderer::onRender(Renderer& renderer) {
    ...
    // leftmost lane edge to the rightmost, near plane to vanishing point.
    {
        ...
        glm::vec2 sFR = w2s({rightX, 0.f, m_farCullZ}, m_perspVP, sw, sh);
        glm::vec2 sFL = w2s({leftX,  0.f, m_farCullZ}, m_perspVP, sw, sh);
    ...
    for (int i = 0; i <= m_laneCount; ++i) {
        ...
        glm::vec2 farPt  = w2s({wx, 0.f, m_farCullZ}, m_perspVP, sw, sh);
    ...
    for (auto& note : m_notes) {
        ...
        const float zFar  = m_farCullZ - 2.f;
        ...
        for (const auto& sp : hold->samplePoints) {
            ...
            float wx   = laneToWorldX(lane);
            float wz   = -static_cast<float>(absT - m_songTime) * (SCROLL_SPEED * m_noteSpeedMul);
            if (wz > 12.f || wz < m_farCullZ - 1.f) continue;
    ...
    for (auto& note : m_notes) {
        ...
        if (noteZ > upperClip || noteZ < m_farCullZ - 2.f) continue;
...
void Drop2DRenderer::showJudgment(int lane, Judgment judgment, float timingDelta) {
...
void Drop2DRenderer::showHitEffect(HitEventKind kind, int lane,
...
void Drop2DRenderer::emitHoldAura(float dt) {
...
void Drop2DRenderer::onShutdown(Renderer& renderer) {
    m_renderer = nullptr;
    m_notes.clear();
    m_hitNotes.clear();
