...
// effect should fire. Bandori 2D-drop is the proving-ground mode; other modes
...
// Bandori event slots, in display order.
...
// Cytus (ScanLine) event slots. Adds `slide_tick` — every slide sample point
// that the scan line passes / the finger tracks (ScanLine-only note type).
const std::vector<ParticleSlotInfo>& cytusParticleSlots();

// Arcaea (3D drop) event slots. `click_hit` = ground lane taps; `sky_hit` =
// arc / arc-tap (sky) events. NOTE: arcaea effects emit in WORLD space, so the
// seeded defaults use world-scale speed/size (see defaultForSlot).
const std::vector<ParticleSlotInfo>& arcaeaParticleSlots();

// Lanota (Circle/radial) event slots. Emits in screen space like Bandori.
const std::vector<ParticleSlotInfo>& lanotaParticleSlots();

// Slot table for a mode string ("bandori"/"cytus"/"arcaea"/"lanota"). Falls
// back to Bandori.
