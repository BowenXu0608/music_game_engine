...
// PBR (Disney metallic-roughness subset) for the 2D quad path.
//
// Push-constant block reinterprets the shared 128 B layout:
//   model      mat4   model matrix
//   baseColor  vec4   rgb + alpha   (== legacy `tint` slot)
//   mrp        vec4   x=metallic y=roughness z=emissiveIntensity w=flags
//                                   (== legacy `uvTransform` slot)
//   emissive   vec4   rgb emissive colour   (== legacy `params` slot)
//   kind       uint   PBR sentinel
//
// A flat camera-facing quad has a constant geometric normal, so with no
// normal map the surface is smoothly lit and only alpha varies. A normal map
// (flags bit0) gives real per-pixel relief via a screen-space-derivative TBN
// (no tangent vertex attribute needed).
...
void main() {
    ...
    // Constant flat normal for a camera-facing quad, oriented toward the eye.
    vec3 V = normalize(ubo.cameraPos.xyz - fragWorldPos);
    vec3 Ng = normalize(cross(dFdx(fragWorldPos), dFdy(fragWorldPos)));
