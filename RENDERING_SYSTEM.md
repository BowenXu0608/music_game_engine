...
## Hub Mode Rendering (2026-03-30)

### Overview
Engine now supports dual rendering modes: Hub and Editor.

### Hub Mode
- Project browser UI
- Scans `Projects/` directory for `project.json` files
- Lists projects with name and version
- Click to dynamically load project

### Editor Mode
- Scene viewer with Play/Stop controls
- Stats window (FPS, timing)
- Game renders to offscreen texture with bloom
- ImGui overlay

### Implementation
**Engine changes:**
- Added `m_hubMode` flag and `m_hub` member
- `runHub()` method for hub launcher
- `render()` switches UI based on mode

**Window:**
- `GLFW_RESIZABLE = GLFW_TRUE`
- Hub UI fills viewport, no restrictions

**Files:**
- `engine/src/engine/Engine.h/.cpp`
- `engine/src/ui/ProjectHub.h/.cpp`
- `engine/src/main.cpp`
